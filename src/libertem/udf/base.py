...
class UDFRunner:
    ...
    def _run_tile(self, partition, udfs, tile, device_tile):
        for udf in udfs:
    ...
    def _run_udfs(self, numpy_udfs, cupy_udfs, partition, tiling_scheme, roi, dtype):
        ...
        run_tile = self._run_tile
        for tile in tiles:
            run_tile(partition, numpy_udfs, tile, tile)
            if cupy_udfs:
                # Work-around, should come from dataset later
                device_tile = xp.asanyarray(tile)
                run_tile(partition, cupy_udfs, tile, device_tile)
