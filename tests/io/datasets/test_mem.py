...
from utils import _mk_random, dataset_correction_verification, set_device_class
...
def test_correction(lt_ctx, with_roi, backend):
    if backend in CUPY_BACKENDS and not has_cupy:
        pytest.skip("No CUDA device or no CuPy, skipping CuPy test")
    data = _mk_random(size=(16, 16, 16, 16))
    ds = MemoryDataSet(
...
def test_positive_sync_offset_with_roi(lt_ctx, backend):
    if backend in CUPY_BACKENDS and not has_cupy:
        pytest.skip("No CUDA device or no CuPy, skipping CuPy test")
    if backend in CUDA_BACKENDS:
        with set_device_class('cupy'):
...
def test_negative_sync_offset_with_roi(lt_ctx, backend):
    if backend in CUPY_BACKENDS and not has_cupy:
        pytest.skip("No CUDA device or no CuPy, skipping CuPy test")
    if backend in CUDA_BACKENDS:
        with set_device_class('cupy'):
